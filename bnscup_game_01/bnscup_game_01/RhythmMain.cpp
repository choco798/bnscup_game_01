...
class MoraRenderer
{
   public:
	...
	void drawCuts(const ParsedStream& P, const Layout& L,
				  double scrollBeat) const;
	void drawMoras(const ParsedStream& P, const Layout& L, double scrollBeat,
				   double voiceLevel) const;

   private:
	void drawMoraGlyph(const Mora& m, const Layout& L, double x,
...
void DrawCut(double x, const Layout& L, bool strong)
{
	...
	Line{x, L.laneY - len * 0.5, x, L.laneY + len * 0.5}.draw(
...
void MoraRenderer::drawCuts(const ParsedStream& P, const Layout& L,
							double scrollBeat) const
{
...
void MoraRenderer::drawMoras(const ParsedStream& P, const Layout& L,
							 double scrollBeat, double voiceLevel) const
{
	...
	for (size_t i = 0; i < P.moras.size(); ++i)
	{
		...
		{
			const double nx =
				L.hitX + (P.moras[i + 1].startBeat - scrollBeat) * L.pxPerBeat;
			if (nx > x + 24) DrawBeam(x, nx, L);
		}
...
void MoraRenderer::drawMoraGlyph(const Mora& m, const Layout& L, double x,
								 const ColorF& glow) const
{
	// 声量エフェクト：背景グロー
	if (glow.a > 0.01)
	{
		Circle{x, L.laneY, L.moraRadius * (1.0 + 0.8 * glow.a)}.drawShadow(
			Vec2{0, 0}, 8, 4, glow);
	}
	...
	const Font& f = (m.text.size() >= 2 ? FontAsset(U"KANA_SMALL")
										: FontAsset(U"KANA_BIG"));
	f(m.text).drawAt(Vec2{x, L.laneY + 26}, Palette::Black);

	// フェルマータ
	if (m.fermata) DrawFermata(x, L);
}
...
class VoiceReactiveFx
{
   public:
	...
	{
		// 簡易：エネルギー正規化 → 平滑 → 0..1
		double target = Saturate((s.energy - s.noise + (s.active ? 0.5 : 0)) /
								 (s.noise * 4.0 + 0.1 + 0.5));
...
class VoiceActivityDetector : public IVoiceActivityDetector
{
   public:
	...
	VoiceActivityDetector(const Optional<uint32>& deviceIndex = unspecified,
		...
		: m_params(params), m_dbgOpt(dbg)
	{
		...
		if (m_dbgOpt.enabled)
		{
			...
			if (m_writer.open(m_dbgOpt.logPath, mode))
			{
				...
				{
					// CSVヘッダ
					// time_ms, energy, noiseEMA, onThresh, offThresh, rawVoice,
					// confirmedVoice, i0, i1, hzPerBin, fps
					m_writer.writeln(
	...
	bool update() override
	{
		...
		const int32 i0 =
			Max(0, static_cast<int32>(m_params.bandLowHz / hzPerBin));
		const int32 i1 =
		...
		{
			const int32 needMs =
				rawVoice ? m_params.minOnMs : m_params.minOffMs;
   ...
   private:
				  ...
				  double hzPerBin)
	{
		...
		m_writer.writeln(Format(tms, U",", m_lastEnergy, U",", m_noiseEMA, U",",
								onTh, U",", offTh, U",", (rawVoice ? 1 : 0),
								U",", (m_confirmedVoice ? 1 : 0), U",", i0,
								U",", i1, U",", hzPerBin, U",", fps));
...
class RhythmApp
{
   public:
	...
	void updateAndDraw()
	{
		...
		const double nowBeat =
			clampBeat(m_transport.nowBeat(), m_parsed.totalBeats);

		// モーラ通過イベント（SE）
		m_hit.process(m_parsed, nowBeat,
					  [&](size_t i) { m_se.playOneShot(0.7); });
	...
	void setVAD(std::unique_ptr<IVoiceActivityDetector> vad)
	{
		m_vad = std::move(vad);
	}
   ...
   private:
	...
	void drawUI() const
	{
		FontAsset(U"UI_SMALL")(
			U"[SPACE] 再生/一時停止  [R] リセット  BPM:{}"_fmt(
				m_transport.bpm()))
